var numpad = {
  ...
  hsubmit: null, // individual buttons
  init: function(){
    // (A1) WRAPPER
    ...
    let buttonator = function (txt, css, fn) {
      ...
      if (txt == "&#10004;"){
      button.setAttribute("name", "numpad_ok")
      }
  ...
  // (B6) OK - SET VALUE
  select: function(){
    if(numpad.autosubmittarget != null){
      numpad.autosubmittarget.submit();
    }
  ...
  //  dash: optional, allow dash? Default false.

    // (C1) DEFAULT OPTIONS
    if (opt.max === undefined) { opt.max = 255; }
    if (opt.decimal === undefined) { opt.decimal = true; }
    if (opt.dash === undefined) { opt.dash = false; }
    if (opt.autosubmittarget === undefined) {opt.autosubmittarget = false}
    ...
    target.dataset.dash = opt.dash;
    target.dataset.autosubmittarget = opt.autosubmittarget;
    target.addEventListener("dblclick", numpad.show);
  },
  ...
  show: function() {
    ...
    // (D3) SET DASH
    if (this.dataset.dash == "true") {
      numpad.hbwrap.classList.remove("noDash");
    } else {
      numpad.hbwrap.classList.add("noDash");
    }

    // (D4) SET CURRENT TARGET
    numpad.nowTarget = this;

    // (D4.5) SET AUTOSUBMIT TARGET
    if (this.dataset.autosubmittarget != "false") {
      numpad.autosubmittarget = document.getElementById(this.dataset.autosubmittarget);
    }
